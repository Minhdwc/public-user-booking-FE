      ...
      <div className="relative aspect-16/10 overflow-hidden bg-muted">
        ...
        {hasRating ? (
          <div className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full border border-white/20 bg-black/40 px-2.5 py-1 text-xs font-semibold text-amber-300 backdrop-blur-md shadow-xs">
            ...
            {venue.ratingCount ? <span className="text-[10px] text-white/70">({venue.ratingCount})</span> : null}
          </div>
        ) : null}
        ...
        {minPrice !== null ? (
          <div className="absolute bottom-3 left-3 flex items-baseline gap-1 rounded-xl border border-white/20 bg-emerald-600/90 px-3 py-1 text-sm font-extrabold text-white shadow-md backdrop-blur-md">
            ...
            <span className="text-[11px] font-normal text-emerald-100">/giờ</span>
          </div>
        ) : null}
      </div>
      ...
      <div className="flex flex-1 flex-col justify-between space-y-3 p-4">
        <div>
          ...
          <p className="mt-1.5 flex items-start gap-1.5 text-xs text-muted-foreground">
            ...
            <span className="line-clamp-2 leading-relaxed">{venue.location || 'Chưa có địa chỉ'}</span>
          </p>
        </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-border/50 pt-3">
          <div className="flex flex-wrap items-center gap-1.5">
            {sports.slice(0, 2).map((sport) => (
              <span
                key={sport.id}
                className="rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-[11px] font-medium text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-300"
              >
                {sport.name}
              </span>
            ))}
            {sports.length > 2 ? (
              <span className="text-[11px] font-medium text-muted-foreground">+{sports.length - 2}</span>
            ) : null}
          </div>
