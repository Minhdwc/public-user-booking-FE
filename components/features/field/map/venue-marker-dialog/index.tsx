...
export function VenueMapMarkerDialog({ venue, open, onOpenChange }: VenueMapMarkerDialogProps) {
  ...
  const coverImage = venueDetail?.venueImages?.[0]?.url;
  const address = venueDetail ? formatVenueAddress(venueDetail) : venue?.location;
  const operatingHour = venueDetail?.operatingHours?.[0];
              ...
              <div className="space-y-3 text-sm">
                ...
                {operatingHour?.openTime && operatingHour?.closeTime ? (
                  <p className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>
                      Mở cửa {formatTime(operatingHour.openTime)} –{' '}
